...
快手Cookie：
  ...
  ^http://uploads2\.gifshow\.com/rest/n/system/speed url script-request-header NE.cookie.js
...
!(async () => {
  if ($.isRewrite && $request.method != 'OPTIONS') {
    ...
    if (result < 0) {
      ...
      $.setdata(JSON.stringify(newCks, null, ' '), 'CookiesJD');
    }
    $.log('京东Cookie 整理操作完成');
    ...
    if (result < 0) {
      ...
      $.setdata(JSON.stringify(ckArr, null, ' '), 'cookies_ks');
    }
    $.log('快手Cookie 整理操作完成');
  }
      ...
      if ($request.headers) {
        ...
        if (!acObj.cookie) {
          ...
          if (seatNo) {
            ...
            if (no <= 2) {
              $.setdata(acObj.cookie, no == 2 ? 'CookieJD2' : 'CookieJD');
            } else {
              ...
              $.setdata(JSON.stringify(oldCks, null, ' '), 'CookiesJD');
            }
...
function chooseSeatNo(newCk, oldCks, reg) {
  let seatNo = 0; // 存储位置，小于0更新、等于0不操作、大于0新增
  try {
    ...
    for (let i = 0, len = oldCks.length; i < len; i++) {
      ...
      if (oldId) {
        ...
        // 旧cookie无效且未分配到账号位，先标记新cookie数据存储于此位置
        seatNo = i + 1;
      }
    }
  } catch (e) {
  ...
  return new Promise(resolve => {
    try {
      if ($request.headers) {
        let acObj = {};
        let ck = ($request.headers['Cookie'] || $request.headers['cookie'] || '').replace(/ /g, '') + ';';
        let ckItems = ck.split(';').filter(s => /^(client_key|kpf|kpn|kuaishou.api_st|kuaishou.h5_st|kuaishou.sixin.login_st|userId|token)=.+/.test(s)).sort().reverse();
        const newCk = ckItems.join('; ') + ';';
        acObj.userId = (newCk.match(/userId=(.+?);/) || ['', ''])[1];
        let kpn = (newCk.match(/kpn=(.+?);/) || ['', ''])[1];
        if (kpn == 'NEBULA' && ckItems.length >= 6 && acObj.userId) {
          // 快手极速版
          acObj.cookie = newCk;
        } else if (kpn == 'KUAISHOU' && ckItems.length >= 7 && acObj.userId) {
          // 快手官方版
          acObj.cookie = newCk;
        }
